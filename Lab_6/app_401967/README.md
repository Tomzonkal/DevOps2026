...
![Wynik docker images grep kalkulator — kalkulator-cache 415MB, kalkulator-slim 51.1MB](screenshots/screen_6.png)
...
![Wynik docker images kalkulator-final — 47.1 MB, 8.976s; wyniki curl /health, /calculate i dzielenia przez zero](screenshots/screen_7.png)
![](screenshots/screen_8.png)
![](screenshots/screen_9.png)
![](screenshots/screen_10.png)
 
Finalny obraz po multi-stage build: **47.1 MB**, czas buildu z cache **8.976 s**. Wszystkie endpointy działają poprawnie, dzielenie przez zero zwraca HTTP 400 z komunikatem `{"error": "Dzielenie przez zero"}`.
...
```bash
xcopy Lab_6\app_0000 Lab_6\app_401967 /E /I
cd Lab_6/app_401967
```

Cała dalsza praca odbywa się wewnątrz folderu `app_401967`, bez modyfikacji oryginału `app_0000`.

---

### Krok 3 – Pomiar stanu bazowego (baseline)

#### Krok 3.1 – Budowa obrazu baseline i pomiar czasu
...
#### Krok 3.2 – Rozmiar obrazu baseline
...
#### Krok 3.3 – Weryfikacja działania aplikacji

```bash
docker run -d --name kalkulator-test -p 5000:5000 kalkulator-baseline
curl -X POST http://localhost:5000/calculate -H "Content-Type: application/json" -d "{\"a\": 10, \"op\": \"+\", \"b\": 5}"
...
Obraz `python:3.11` to pełny obraz Debian z kompilatorem, narzędziami build i wieloma bibliotekami których aplikacja nie potrzebuje. Jego rozmiar to ~1 GB.
...
curl -X POST http://localhost:5000/calculate -H "Content-Type: application/json" -d "{\"a\": 7, \"op\": \"*\", \"b\": 6}"
...
**Rozmiar finalnego obrazu: 47.1 MB**, czas budowania : **8.976 sekundy** (z cache 2.667 s).
...
```bash
curl -X POST http://localhost:5000/calculate -H "Content-Type: application/json" -d "{\"a\": 10, \"op\": \"/\", \"b\": 0}"
```
Wynik: `{"error": "Dzielenie przez zero"}` 
...
Łączna redukcja rozmiaru: **419 MB → 47.1 MB**.  
Łączna redukcja czasu buildu (z cache): **14 s → 2.667 s**.
