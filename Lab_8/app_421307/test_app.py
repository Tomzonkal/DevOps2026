...
def test_uppercase_missing_field():
    r = requests.post(f"{BASE_URL}/uppercase", json={"wrong": "hello"})
...
def test_reverse_missing_field():
    r = requests.post(f"{BASE_URL}/reverse", json={"wrong": "hello"})
    assert r.status_code == 400
    assert "error" in r.json()
...
def test_word_count_missing_field():
    r = requests.post(f"{BASE_URL}/word-count", json={"wrong": "hello"})
    assert r.status_code == 400
    assert "error" in r.json()
