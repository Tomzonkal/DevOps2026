...
def test_subtract():
    response = requests.post(f"{BASE_URL}/subtract", json={"a": 10, "b": 5})
    assert response.status_code == 200
    assert response.json()["result"] == 5


def test_multiply():
    response = requests.post(f"{BASE_URL}/multiply", json={"a": 10, "b": 5})
    assert response.status_code == 200
    assert response.json()["result"] == 50


def test_divide():
    response = requests.post(f"{BASE_URL}/divide", json={"a": 10, "b": 5})
    assert response.status_code == 200
